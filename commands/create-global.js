...
module.exports = {
    ...
    .addStringOption((option) =>
      option
        ...
        .addChoices(
          ...
          {
            name: "Long Date/Time (e.g., Wednesday, 13 June 2024 9:41 PM)",
            value: "F",
          },
          {
            name: "Relative Time (e.g., in 8 hours / 2 months ago)",
            value: "R",
          }
        )
    )
  ...
  async execute(interaction) {
    ...
    const timezone =
      interaction.options.getString("timezone") || localTimezone();

    // Create Date object and get Unix timestamp
    const date = new Date(Date.UTC(year, month - 1, day, hours, minutes));
    const offsetMinutes =
      parseInt(timezone.slice(1, 3)) * 60 + parseInt(timezone.slice(4));
    date.setMinutes(
      date.getMinutes() -
        (timezone.startsWith("-") ? -offsetMinutes : offsetMinutes)
    );
  ...
  async autocomplete(interaction) {
    ...
    const filteredCities = timezones.flatMap((choice) =>
      choice.cities
        ...
        .map((city) => ({
          name: `${city} - ${choice.name}`,
          value: choice.value,
        }))
    );

    const filtered = [
      ...filteredTimezones.map((choice) => ({
        name: choice.name,
        value: choice.value,
      })),
      ...filteredCities,
